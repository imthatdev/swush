...
export default function BookmarkTagsDialog({
  username,
  onChanged,
}: {
  username: string;
  ...
  const [items, setItems] = useState<BookmarkTagItem[]>([]);
  ...
  const loadTags = useCallback(async () => {
    try {
      ...
      const data = (await res.json()) as BookmarkTagItem[];
      setItems(data);
    } catch (err) {
  ...
  const createTag = async () => {
    ...
    try {
      setSaving(true);
      const res = await fetch(apiV1("/bookmark-tags"), {
        method: "POST",
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Create, edit, share, or remove bookmark tags.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between rounded-md border p-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Add a colored tag for your bookmarks.
            </div>
          </div>
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Create bookmark tag</DialogTitle>
                <DialogDescription>
              ...
              <div className="grid gap-3">
                <div className="grid gap-2">
                  <Label htmlFor="bookmark-tag-name">Tag name</Label>
                  <Input
        ...
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Update the name and color for this bookmark tag.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-3">
              <Label htmlFor={`bookmark-tag-${tag.id}`}>Tag name</Label>
              <Input
                id={`bookmark-tag-${tag.id}`}
                value={name}
                onChange={(e) => setName(e.target.value)}
              />
              <ColorPicker
                id={`bookmark-tag-color-${tag.id}`}
                label="Tag color"
                value={color}
                onChange={setColor}
                disabled={working}
