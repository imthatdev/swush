...
export const GET = withApiError(async function GET() {
  ...
  const list = await db
    .select({
      ...
      color: bookmarkTags.color,
      bookmarkCount: count(bookmarks.id).mapWith(Number),
    })
    .from(bookmarkTags)
    .leftJoin(
      bookmarks,
      and(
        ...
        sql`${bookmarks.tags} @> ${sql`ARRAY[${bookmarkTags.name}]`}`,
      ),
    )
    .where(eq(bookmarkTags.userId, user.id))
    .groupBy(bookmarkTags.id, bookmarkTags.name, bookmarkTags.color)
    .orderBy(bookmarkTags.name);

  await audit({
    action: "bookmark_tag.list",
    targetType: "bookmark_tag",
    targetId: user.id,
    statusCode: 200,
...
export const POST = withApiError(async function POST(req: NextRequest) {
  ...
  const usrRL = await rateLimit({
    key: `u:${user.id}:bookmark-tag-create`,
    limit: userLimit,
    windowMs,
  });
  ...
  const exists = await db
    .select()
    .from(bookmarkTags)
    .where(and(eq(bookmarkTags.userId, user.id), eq(bookmarkTags.name, name)))
    .limit(1);
  if (exists.length > 0) {
    return NextResponse.json(
...
export const PATCH = withApiError(async function PATCH(req: Request) {
  ...
  const ipRL = await rateLimit({ key: `ip:${ip}`, limit: ipLimit, windowMs });
  const usrRL = await rateLimit({
    key: `u:${user.id}:bookmark-tag-update`,
    limit: userLimit,
    windowMs,
  });
  ...
  const [existing] = await db
    .select()
    .from(bookmarkTags)
    .where(and(eq(bookmarkTags.id, id), eq(bookmarkTags.userId, user.id)))
    .limit(1);
  if (!existing) {
  ...
  if (name) {
    const duplicate = await db
      ...
      .where(
        and(
          eq(bookmarkTags.userId, user.id),
          eq(bookmarkTags.name, name),
          sql`${bookmarkTags.id} <> ${id}`,
        ),
      )
      .limit(1);
    ...
    if (name && name !== existing.name) {
      await tx.execute(sql`
        ...
        where ${bookmarks.userId} = ${user.id}
          and ${bookmarks.tags} @> ${sql`ARRAY[${existing.name}]`}
      `);
    }
...
export const DELETE = withApiError(async function DELETE(req: Request) {
  ...
  const ipRL = await rateLimit({ key: `ip:${ip}`, limit: ipLimit, windowMs });
  const usrRL = await rateLimit({
    key: `u:${user.id}:bookmark-tag-delete`,
    limit: userLimit,
    windowMs,
  });
  ...
  const [existing] = await db
    .select()
    .from(bookmarkTags)
    .where(and(eq(bookmarkTags.id, body.id), eq(bookmarkTags.userId, user.id)))
    .limit(1);
  if (!existing) {
    return NextResponse.json({ message: "Tag not found" }, { status: 404 });
  }
  ...
  await db.transaction(async (tx) => {
    await tx.delete(bookmarkTags).where(eq(bookmarkTags.id, body.id));
    await tx.execute(sql`
      update ${bookmarks}
